...
3. Create a virtual env, activate it and install the requirements from requirements.txt
   python3 -m venv vehiclevenv python=3.10 -y
   source vehiclevenv/bin/activate
...
19. Complete the work on src.utils.main_utils.py and config.schema.yaml file (add entire info about dataset for data validation step)
...
23. Before moving to next component of Model Evaluation, some AWS services setup is needed:
      ...
      * Go to S3 service >> Create bucket >> Region: us-east-1 >> General purpose >>
        Bucket Name: "my-model-mlopsproj-1" >> uncheck: "Block all public access" and acknowledge >>
        Hit Create Bucket
